...
export default ({ route, navigation }) => {
    ...
    async function handlerAddEvoluction(){
        if(evolucao === ''){
            Alert.alert('Erro', 'Informe um valor para cadastrar!')
        }else{
            try{
    ...
    async function handlerDeleteEvolucao(id) {

        Alert.alert('Excluir Evolução', 'Deseja realmente excluir esta evolução?', [
            {
                text: 'Sim',
                onPress: async () => {
                    const realm = await getRealm();
                    realm.write(async () => {
                        let evo = realm.create('evolution', {id: id}, 'modified');
                        realm.delete(evo);
                    })
                    loadAuthorizations();
                }
    ...
    async function loadAuthorizations(){
        console.log('chamou o loadAuthorization')
        const realm = await getRealm();
        let autorizacao = realm.objects('autorizations');
        let aut = autorizacao.filtered(`id = ${id}`);
    ...
    const Item = ({ title, id }) => (
        <ViewList >
            ...
            <Button 
                ...
                onPress={() => handlerDeleteEvolucao(id)}
            />
        </ViewList>
    );
